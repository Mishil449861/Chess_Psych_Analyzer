...
def name_cluster_with_llm(
    ...
    prompt = f"""You are an experienced chess coach naming a specific tactical/positional pattern.
Below are {len(example_features)} similar mistakes from one student.
...
{examples_text}
RULES:
- The NAME must reference a concrete chess mechanic (piece, square, phase, opening type).
- DO NOT use generic psychology phrases like "Delayed Response Syndrome",
  "Pushed Decision Making", "Loss of Objectivity", "Critical Miscalculation",
  or anything that sounds like behavior coaching rather than chess.
- The DESCRIPTION must mention WHICH PIECE on WHICH KIND OF SQUARE or in WHICH
  POSITION TYPE — not just abstract qualities.
- If the data is too vague to identify a specific chess pattern, respond exactly:
  NAME: Unclear pattern
  DESCRIPTION: Cluster lacks a single dominant chess theme.

Good examples:
  NAME: Hanging knight on the rim
  DESCRIPTION: Leaves a knight on the edge of the board defended only by a pawn.
  NAME: Premature queen sortie
  DESCRIPTION: Brings the queen out early and loses tempo to minor-piece attacks.

Bad examples (DO NOT IMITATE):
  NAME: Delayed Response Syndrome
  DESCRIPTION: The student consistently delays their response.
