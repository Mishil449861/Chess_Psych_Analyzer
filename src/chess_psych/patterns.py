...
def cluster_blunders(
    ...
    Two changes from naive HDBSCAN:
      1. min_cluster_size auto-scales with total blunder count when not given.
         A min of 3 is fine for 30 blunders but produces noise mega-clusters
         on 1000+ blunders.
      2. Clusters larger than 30% of total are rejected as noise. Real
         patterns are specific — a single cluster covering a third of your
         blunders is almost always the HDBSCAN "everything else" bucket.
    """
    ...
    # Auto-scale when not explicitly set
    if min_cluster_size is None:
        if n < 30:
            min_cluster_size = config.cluster_min_size
        else:
            # Scale: ~3% of total, floor 5, ceil 30
            min_cluster_size = max(5, min(30, n // 30))
    ...
    # cluster_selection_method='leaf' tends to give tighter, more specific clusters
    # than the default 'eom' on noisy data.
    labels = HDBSCAN(
    ...
    # Reject clusters that are too large to be specific patterns
    max_cluster_size = int(n * 0.30)
    ...
    # Drop oversized buckets — they're not patterns, they're noise
    rejected = []
    for cid, members in list(clusters.items()):
        if len(members) > max_cluster_size:
            log.info("Rejecting mega-cluster %d (size=%d > %d)",
                     cid, len(members), max_cluster_size)
            rejected.append(cid)
            del clusters[cid]
    n_rejected_members = sum(1 for label in labels if int(label) in rejected)
    ...
    n_noise = int((labels == -1).sum()) + n_rejected_members
    log.info("Clustering done: %d clusters kept, %d noise points (%d from rejected mega-clusters)",
             len(cluster_summaries), n_noise, n_rejected_members)
